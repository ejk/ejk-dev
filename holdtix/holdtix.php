...
if (!empty($item[pemail])) {
  ...
  if (preg_match('/pitts/i', strtolower($pickuploc))) {
    $locker_ad_percentage = 100; // integer between 1 and 100
    if (rand(0,99) < $locker_ad_percentage) {
      exec("echo " . date("Y-m-d H:i:s") . " :: " . $item[pemail] . " >> /tmp/lockerinvite.log");
...
function hold_item_info($email) {
  ...
  $puloc = strtolower($item[pickuploc]);

  // Make sure printer defs match this:
  if (preg_match('/downtown/i', $puloc)) { $item[printer] = 'zebra1'; }
  if (preg_match('/west/i', $puloc)) { $item[printer] = 'zebra4'; }
  if (preg_match('/mallett/i', $puloc)) { $item[printer] = 'zebra3'; }
  if (preg_match('/north/i', $puloc)) { $item[printer] = 'zebra5'; }
  if (preg_match('/pitts/i', $puloc)) { $item[printer] = 'zebra6'; }
  if (preg_match('/traver/i', $puloc)) { $item[printer] = 'zebra8'; }
